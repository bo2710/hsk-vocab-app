// filepath: src/components/exams/ExamSessionQuestionGridPanel.tsx
// CẦN TẠO MỚI
...
export const ExamSessionQuestionGridPanel: React.FC<Props> = ({
  isOpen, onClose, questions, sections, responses, currentIndex, onJump
}) => {
  const { gridData, stats } = useExamQuestionNavigator('session', questions, sections, responses, currentIndex);
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-96 bg-white dark:bg-gray-900 shadow-2xl z-50 transform transition-transform duration-300 flex flex-col border-l border-gray-200 dark:border-gray-800 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
          <h3 className="font-bold text-gray-900 dark:text-white">Bảng Câu Hỏi</h3>
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800 flex justify-between text-sm">
          <div className="text-center">
            <div className="font-bold text-gray-900 dark:text-white">{stats.answered}/{stats.total}</div>
            <div className="text-xs text-gray-500">Đã làm</div>
          </div>
          <div className="text-center">
            <div className="font-bold text-yellow-600 dark:text-yellow-400">{stats.marked}</div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <ExamQuestionNavigatorGrid sections={gridData} onJump={onJump} mode="session" />
        </div>

        <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900">
          <ExamQuestionNavigatorLegend mode="session" />
        </div>
      </div>
    </>
  );
